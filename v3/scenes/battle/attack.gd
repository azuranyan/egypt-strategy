...
func get_target_range(unit_range: int) -> PackedVector2Array:
	return [] #TODO
...
func is_attack_target(user: UnitState, other: UnitState) -> bool:
...
func _execute_later(state: AttackState, effect: AttackEffect, target: UnitState, captures: Dictionary):
	var wrapped := func():
		captures.counter += 1
