...
func _init(_unit_type: UnitType = null, prop := {}):
	if not _unit_type:
		_unit_type = load("res://scenes/battle/unit/data/placeholder_unit_type.tres")
...
func get_pathable_cells(context: BattleContext, use_mov_stat := false) -> PackedVector2Array:
	var cond := func(x):
		return is_pathable(context, x)
	return Util.flood_fill(cell(), stats.mov if use_mov_stat else 20, context.world_bounds(), cond)


## Returns true if cell is pathable.
func is_pathable(context: BattleContext, _cell: Vector2) -> bool:
	for obj in context.get_objects_at(_cell):
		if PathableComponent.GROUP_ID not in obj.components:
			continue
		var pathable := obj[PathableComponent.GROUP_ID] as PathableComponent
		if not pathable.is_pathable(context, self):
			return false
	return true


## Returns true if cell is placeable.
func is_placeable(context: BattleContext, _cell: Vector2) -> bool:
	return is_pathable(context, _cell) and not context.is_occupied(_cell)
